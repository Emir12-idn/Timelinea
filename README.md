...
- **Sheet `Tasks`**: isi Task Name, Level, Duration (hari kerja), Predecessors, % Complete, Assigned To, Cost/Day, dan centang Milestone bila perlu.
  ...
  - **Assigned To** punya dropdown yang otomatis berisi semua nama di sheet `Resources` (klik sel untuk memilih, tidak perlu ketik) — dropdown ini langsung mengikuti isi Resources terkini, jadi menambah orang baru di Resources otomatis muncul sebagai pilihan tanpa langkah tambahan. Untuk menugaskan **lebih dari satu orang** di task yang sama, tetap ketik manual dipisah koma (mis. `Subur, Ade`) — dropdown Sheets tidak mendukung pilih-banyak sekaligus, jadi ketikan manual tetap diterima untuk kasus ini. Nama yang tidak ditemukan di Resources (typo, atau belum ditambahkan) akan muncul sebagai notifikasi toast, dan tidak dihitung biayanya sampai diperbaiki/ditambahkan.

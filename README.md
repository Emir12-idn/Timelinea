...
- **Sheet `Tasks`**: isi Task Name, Level, Duration (hari kerja), Predecessors, % Complete, Assigned To, Cost/Day, dan centang Milestone bila perlu.
  ...
  - **Assigned To** punya dropdown yang otomatis berisi semua nama di sheet `Resources` (klik sel untuk memilih 1 nama, tidak perlu ketik) — dropdown ini langsung mengikuti isi Resources terkini, jadi menambah orang baru di Resources otomatis muncul sebagai pilihan tanpa langkah tambahan. Untuk menugaskan **lebih dari satu orang** di task yang sama, pilih dulu baris task-nya lalu **Timelinea > Pilih Assigned To (Multi-pilih)** — muncul pop-up daftar nama dengan checkbox, tinggal centang siapa saja yang ditugaskan lalu Simpan (tidak perlu ketik manual sama sekali). Nama yang tidak ditemukan di Resources (typo, atau belum ditambahkan) akan muncul sebagai notifikasi toast, dan tidak dihitung biayanya sampai diperbaiki/ditambahkan.
...
- **Timelinea > Print: Sembunyikan Kolom Kerja** menyembunyikan kolom Predecessors s/d Ada Masalah? (kolom-kolom kerja internal: predecessor, cost, critical/slack, baseline/variance, flag masalah) supaya Tasks bisa di-print sebagai daftar ringkas (ID/Task Name/Level/Duration/Start/Finish + Gantt chart) untuk dibagikan ke klien/tim lapangan. Tidak ada data yang dihapus — cuma disembunyikan. **Print: Tampilkan Semua Kolom Lagi** mengembalikannya.

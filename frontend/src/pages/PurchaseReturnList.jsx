...
function NewPurchaseReturnForm({ invoices, items, onClose, onCreated }) {
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const [purchaseInvoiceId, setPurchaseInvoiceId] = useState(invoices[0]?.id ?? "");
  ...
  return (
    <Card className="mb-4 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="text-sm font-semibold text-slate-700">Retur Pembelian Baru</div>
      ...
      <form onSubmit={submit} className="space-y-3">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <Field label="Faktur Pembelian">
            <select required value={purchaseInvoiceId} onChange={(e) => setPurchaseInvoiceId(e.target.value)} className={selectCls}>
              {invoices.map((inv) => <option key={inv.id} value={inv.id}>{inv.no} — {inv.supplier?.name}</option>)}
            </select>
          </Field>
...
export default function PurchaseReturnList() {
  const returns = useApi("/purchase-returns");
  const invoices = useApi("/purchase-invoices");
    ...
    <>
      ...
      {!invoices.loading && (invoices.data || []).length === 0 && (
        <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
          Belum ada Faktur Pembelian untuk diretur.
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-blue-900 text-left text-xs font-semibold uppercase tracking-wide text-white">
                  ...
                  <th className="px-4 py-3">Faktur Asal</th><th className="px-4 py-3">Pemasok</th>
              ...
              <tbody>
                {(returns.data || []).map((r) => (
                  <tr key={r.id} className="border-b border-slate-100 last:border-0 hover:bg-blue-50/50">
                    ...
                    <td className="px-4 py-3 text-slate-600">{r.purchaseInvoice?.no}</td>
                    <td className="px-4 py-3 text-slate-700">{r.purchaseInvoice?.supplier?.name}</td>
                ...
                {(returns.data || []).length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-slate-400">Belum ada retur pembelian.</td></tr>
                )}
              </tbody>
            </table>
          </div>

...
export function slipGajiHtml(payslip: PayslipForPrint): string {
  ...
  const body = `
    ...
    <table>
      <tr>
        <td style="width:50%;vertical-align:top;padding-right:16px;">
          <div style="border-bottom:1px solid #cbd5e1;padding-bottom:4px;margin-bottom:4px;font-weight:700;color:#1e3a6e;">Pendapatan</div>
          ${row("Gaji Pokok", payslip.baseSalary)}
          ${row("Tunjangan", payslip.allowance)}
          ${row("Lembur", payslip.overtimeAmount)}
          <div style="display:flex;justify-content:space-between;border-top:1px solid #cbd5e1;margin-top:4px;padding-top:4px;font-weight:700;">
            <span>Bruto</span><span>${rupiah(payslip.gross)}</span>
          </div>
        </td>
        <td style="width:50%;vertical-align:top;">
          <div style="border-bottom:1px solid #cbd5e1;padding-bottom:4px;margin-bottom:4px;font-weight:700;color:#1e3a6e;">Potongan</div>
    ...
    ${sisa > 0n ? `<div style="margin-top:8px;font-size:11.5px;color:#64748b;">Sisa kasbon/hutang pegawai: ${rupiah(sisa)}</div>` : ""}
    ${ttdBlock("Karyawan,", "HRD / Keuangan,")}
  `;
